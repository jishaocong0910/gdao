...
import (
	"github.com/jishaocong0910/gdao/gen/testdata/internal/sqlserver"
	"testing"
	"time"
...
func TestBaseDao_Get(t *testing.T) {
	...
	{
		d, mock := dao.MockSqlserverBaseDao[User](r, "user")
		mock.ExpectPrepare(`SELECT TOP 1 id,phone FROM user WHERE status=:1 FOR UPDATE`).
			ExpectQuery().WithArgs(4).WillReturnRows(mock.NewRows([]string{"id", "name"}).
			AddRow(1, "lucy"))
...
func TestCondition(t *testing.T) {
	...
	}
	{
		...
		_, _, err := d.Query(gdao.QueryReq[User]{BuildSql: func(b *gdao.Builder[User]) {
			...
			c = c.Group(c2).Group(c3).Group(c4).Group(c5).Group(c6)
			dao.WriteCondition(c, b)
		}})
		r.NoError(err)
	}
