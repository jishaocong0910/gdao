...
import (
	dao "github.com/jishaocong0910/gdao/gen/testdata/sqlserver"
	"testing"
	"time"
...
func TestNewBaseDaoPanic(t *testing.T) {
	...
	r.PanicsWithValue("table must not be blank", func() {
		dao.MockSqlserverBaseDao[User](r, "")
	})
}
...
func TestBaseDao_List(t *testing.T) {
	r := require.New(t)
	{
		d, mock := dao.MockSqlserverBaseDao[User](r, "user")
		mock.ExpectPrepare(`SELECT id,name FROM user WHERE status=:1 ORDER BY name ASC,address DESC LIMIT 10,10 FOR UPDATE`).
...
func TestBaseDao_Get(t *testing.T) {
	...
	{
		d, mock := dao.MockSqlserverBaseDao[User](r, "user")
		mock.ExpectPrepare(`SELECT id,phone FROM user WHERE status=:1 FOR UPDATE`).
			ExpectQuery().WithArgs(4).WillReturnRows(mock.NewRows([]string{"id", "name"}).
			AddRow(1, "lucy"))
...
func TestBaseDao_Insert(t *testing.T) {
	...
	{
		d, mock := dao.MockSqlserverBaseDao[User](r, "user")
		mock.ExpectPrepare(`INSERT INTO user\(name,phone,email,level\) VALUES\(:1,:2,:3,NULL\); SELECT ID=convert\(bigint, SCOPE_IDENTITY\(\)\)`).
...
func TestBaseDao_InsertBatch(t *testing.T) {
	...
	mock.ExpectPrepare(`INSERT INTO user\(name,phone,email\) VALUES\(:1,:2,:3\),\(:4,:5,:6\); SELECT ID=convert\(bigint, SCOPE_IDENTITY\(\)\)`).
		ExpectQuery().WithArgs("abc", "12345", nil, "def", "6789", nil).WillReturnRows(sqlmock.NewRows([]string{"ID"}).AddRow(9))
