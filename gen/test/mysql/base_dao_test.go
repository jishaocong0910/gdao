...
import (
	...
	dao "github.com/jishaocong0910/gdao/gen/test/mysql/internal"
...
func TestBaseDao_List(t *testing.T) {
	r := require.New(t)
	{
		...
		list, err := d.List().Selects("id", "Name").Cond(dao.And().Eq("status", 4)).
...
func TestBaseDao_Get(t *testing.T) {
	r := require.New(t)
	{
		...
		get, err := d.Get().Selects("id", "name").Cond(dao.And().Eq("status", 4)).
	...
	}
	{
		...
		_, err := d.Get().Selects("name").CheckOne(true).Cond(dao.And().Eq("status", 1)).Do()
...
func TestBaseDao_Insert(t *testing.T) {
	...
	{
		d, mock := dao.MockBaseDao[User](r, "user")
		mock.ExpectPrepare(`INSERT IGNORE INTO user\(name, age, address, phone, status, create_at, level\) VALUES\(\?, NULL, NULL, \?, NULL, NULL, NULL\) ON DUPLICATE KEY UPDATE phone = \?, id = id`).
			ExpectExec().WithArgs("abc", "12345", "phone").WillReturnResult(sqlmock.NewResult(7, 1))
		...
		affected, err := d.Insert().Entity(u).All(true).SetNulls("level").
			Ignores("email").InsertIgnore(true).
...
func TestBaseDao_InsertBatch(t *testing.T) {
	...
	affected, err := d.InsertBatch().Entities(u, u2).Do()
...
func TestBaseDao_UpdateBatch(t *testing.T) {
	...
	{
		d, mock := dao.MockBaseDao[User](r, "user")
		mock.ExpectPrepare(`UPDATE user SET name = CASE id WHEN \? THEN \? WHEN \? THEN \? WHEN \? THEN \? END, phone = CASE id WHEN \? THEN \? WHEN \? THEN \? WHEN \? THEN \? END, state = NULL, level = NULL WHERE id IN\(\?, \?, \?\) AND status = \?`).
		...
		affected, err := d.UpdateBatch().Entities(u, u2, u3).SetNulls("state", "level").
			Ignores("email").Where("id").Cond(dao.And().Eq("Status", 1)).Do()
...
func TestBaseDao_Count(t *testing.T) {
	...
	count, err := d.Count().Cond(dao.And().Eq("status", 1)).Do()
