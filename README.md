...
1. 查询数据并映射为实体。
2. 插入数据并将自动生成key映射到实体。
3. 兼容不支持`sql.Result#LastInsertId`方法的驱动。
4. 内置用于自定义SQL的辅助方法。

兼容所有数据库是GDAO的原则，它通过用户自定义SQL来执行，因此兼容所有数据库的方言和驱动的参数占位符。不同数据库驱动在插入数据后，获取自动生成key也是风格迥异，GDAO对此也做了兼容性设计。
...
实体字段类型只支持如下类型的**指针**和切片（可多维切片，支持PostgreSQL）
...
func main() {
    gen.Generator(gen.Config{
        ...
        FieldNameMapper:  nil,                                   // 指定数据库字段名->实体字段名的映射，默认为大驼峰格式
    }).Gen()
}
...
| `Ctx context.Context`                                  | Context              |
| `Tx *sql.Tx`                                           | 事务                   |
| `Entities []*T`                                        | 实体参数，用于`BuildSql`函数。 |
| `BuildSql func(b Builder[T]) (sql string, args []any)` | SQL构建函数              |
...
| `Ctx context.Context`                                  | Context                                                                                |
| `Tx *sql.Tx`                                           | 事务                                                                                     |
| `Entities []*T`                                        | 实体参数，有两个作用：<br/>1. 用于`BuildSql`函数<br/>2. 使用`Insert`或`Query`执行模式，会将自动生成key或返回结果映射回这些实体。 |
| `BuildSql func(b Builder[T]) (sql string, args []any)` | SQL构建函数                                                                                |
...
与Exec模式相同，但多了一个步骤：调用`sql.Result#LastInsertId`方法将自增key映射到实体参数中。**适合用于执行INSERT语句，并且数据库驱动支持`sql.Result#LastInsertId`方法的情况，如果数据库驱动不支持，则映射的自增key将返回error或不准确**。
...
内部调用Golang的`sql.Stmt#QueryContext`方法执行，返回影响行数。**适用于一些不支持`sql.Result#LastInsertId`方法的数据库驱动，在执行INSERT语句后映射自动生成key**。
...
DAO的`Query`和`Mutation`方法中，`BuildeSql`参数是一个函数，要求用户返回自定义SQL和占位符参数。函数的`gdao.Builder`参数是用于构建SQL的工具，它提供了许多方便自定义SQL的方法。
...
| `Entity`             | 返回所属DAO方法的实体参数中第一个实体值。                              |
| `EntityAt`           | 返回所属DAO方法的实体参数中指定索引的实体值，通常配合`EachColumnAt`方法一起使用。   |
| `EachEntity`         | 遍历所属DAO方法的实体参数，其`handle`函数参数的i参数为实体参数索引。            |
| `EachColumn`         | 遍历所属DAO方法的实体参数中第一个实体的所有字段。                          |
| `EachColumnAt`       | 遍历所属DAO方法的实体参数中定索引的实体的所有字段。                         |
| `EachAssignedColumn` | 遍历所属DAO方法的实体参数中第一个实体的所有不为nil的字段。                    |
