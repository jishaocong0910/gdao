...
import (
	...
	"github.com/jishaocong0910/cozy-orm/dial/test/postgres/internal"
...
func TestBaseDao_Insert(t *testing.T) {
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`INSERT INTO user\(name, phone, email, status\) VALUES \(\$1, \$2, \$3, NULL\) RETURNING id`).
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`INSERT INTO user\(name, age, address, phone, status, level, create_at\) VALUES \(\$1, NULL, NULL, \$2, NULL, NULL, NULL\) RETURNING id`).
...
func TestBaseDao_InsertBatch(t *testing.T) {
	r := require.New(t)
	d, mock := testdata.MockBaseDao[User](r, nil)
	mock.ExpectPrepare(`INSERT INTO user\(name, phone, email, level\) VALUES \(\$1, \$2, \$3, \$4\), \(\$5, \$6, \$7, NULL\) RETURNING id`).
		ExpectQuery().WithArgs("abc", "12345", "email11", 2, "def", "6789", "email22").WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(8).AddRow(9))
