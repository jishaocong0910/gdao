...
import (
	...
	"github.com/jishaocong0910/cozy-orm/dial/test/mysql/internal"
...
func TestBaseDao_Insert(t *testing.T) {
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`INSERT IGNORE INTO user\(name, phone, email, status\) VALUES \(\?, \?, \?, NULL\) ON DUPLICATE KEY UPDATE phone = \?, status = 1`).
		...
		affected, err := d.Insert().Entity(u).Nullable("email", "status").InsertIgnore(true).
			OnDuplicateKey(testdata.OnDuplicateKey().Update("phone", "phone").UpdatePlain("status", "1")).
			Do()
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`INSERT IGNORE INTO user\(name, age, address, phone, status, level, create_at\) VALUES \(\?, NULL, NULL, \?, NULL, NULL, NULL\) ON DUPLICATE KEY UPDATE phone = \?, status = 1`).
		...
		affected, err := d.Insert().Entity(u).All(true).Ignore("email").InsertIgnore(true).
			OnDuplicateKey(testdata.OnDuplicateKey().Update("phone", "phone").UpdatePlain("status", "1")).
			Do()
