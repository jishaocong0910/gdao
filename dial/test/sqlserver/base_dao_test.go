...
import (
	...
	"github.com/jishaocong0910/cozy-orm/dial/test/sqlserver/internal"
...
func TestBaseDao_Insert(t *testing.T) {
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`INSERT INTO user\(name, phone, email, status\) VALUES \(:1, :2, :3, NULL\); SELECT ID = convert\(bigint, SCOPE_IDENTITY\(\)\)`).
		...
		err := d.Insert().Entity(u).Nullable("email", "status").Do()
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`INSERT INTO user\(name, age, address, phone, status, level, create_at\) VALUES \(:1, NULL, NULL, :2, NULL, NULL, NULL\)`).
		...
		err := d.Insert().Entity(u).All(true).Ignore("email").Do()
...
func TestBaseDao_InsertBatch(t *testing.T) {
	r := require.New(t)
	d, mock := testdata.MockBaseDao[User](r, nil)
	mock.ExpectPrepare(`INSERT INTO user\(name, phone, email, level\) VALUES \(:1, :2, :3, :4\), \(:5, :6, :7, NULL\); SELECT ID = convert\(bigint, SCOPE_IDENTITY\(\)\)`).
		ExpectQuery().WithArgs("abc", "12345", "email11", 2, "def", "6789", "email22").WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(9))
	...
	err := d.InsertBatch().Entities(u, u2).Nullable("level").Do()
...
func TestBaseDao_Update(t *testing.T) {
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`UPDATE user SET name = :1, phone = :2, email = NULL WHERE status = :3 AND level = :4 AND age = :5`).
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`UPDATE user SET name = :1, age = NULL, address = :2, phone = NULL, level = :3, create_at = NULL WHERE id = :4 AND status IS NULL`).
...
func TestBaseDao_UpdateBatch(t *testing.T) {
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`UPDATE user SET name = CASE id WHEN :1 THEN :2 WHEN :3 THEN :4 WHEN :5 THEN :6 END, phone = CASE id WHEN :7 THEN :8 WHEN :9 THEN :10 WHEN :11 THEN :12 END, level = CASE id WHEN :13 THEN NULL WHEN :14 THEN NULL WHEN :15 THEN NULL END WHERE id IN\(:16, :17, :18\) AND status = :19`).
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`UPDATE user SET name = CASE id WHEN :1 THEN :2 WHEN :3 THEN :4 WHEN :5 THEN :6 END, age = CASE id WHEN :7 THEN NULL WHEN :8 THEN NULL WHEN :9 THEN NULL END, address = CASE id WHEN :10 THEN NULL WHEN :11 THEN NULL WHEN :12 THEN NULL END, phone = CASE id WHEN :13 THEN :14 WHEN :15 THEN :16 WHEN :17 THEN :18 END, email = CASE id WHEN :19 THEN :20 WHEN :21 THEN :22 WHEN :23 THEN :24 END, create_at = CASE id WHEN :25 THEN NULL WHEN :26 THEN NULL WHEN :27 THEN NULL END WHERE id IN\(:28, :29, :30\)`).
...
func TestLogicalDel_SetId(t *testing.T) {
	...
	{
		mock.ExpectPrepare(`UPDATE sku SET deleted = id WHERE id = :1 AND deleted = :2`).
...
func TestLogicalDel_SetNull(t *testing.T) {
	...
	{
		mock.ExpectPrepare(`UPDATE product SET valid = NULL WHERE id = :1 AND valid = :2`).
