...
import (
	...
	"github.com/jishaocong0910/cozy-orm/dial/test/oracle/internal"
...
func TestBaseDao_Insert(t *testing.T) {
	...
	{
		d, mock := testdata.MockBaseDao[User](r, nil)
		mock.ExpectPrepare(`INSERT INTO user\(name, phone, email, status\) VALUES \(:1, :2, :3, NULL\)`).
	...
	}
	{
		...
		mock.ExpectPrepare(`INSERT INTO user\(name, age, address, phone, status, level, create_at\) VALUES \(:1, NULL, NULL, :2, NULL, NULL, NULL\)`).
			ExpectExec().WithArgs("abc", "12345").WillReturnResult(sqlmock.NewResult(0, 1))
		...
		affected, err := d.Insert().Entity(u).All(true).Ignore("id", "email").Do()
...
func TestBaseDao_InsertBatch(t *testing.T) {
	r := require.New(t)
	d, mock := testdata.MockBaseDao[User](r, nil)
	mock.ExpectPrepare(`INSERT INTO user\(name, phone, email, level\) VALUES \(:1, :2, :3, NULL\), \(:4, :5, :6, NULL\)`).
		ExpectExec().WithArgs("abc", "12345", "email11", "def", "6789", "email22").WillReturnResult(sqlmock.NewResult(0, 2))
