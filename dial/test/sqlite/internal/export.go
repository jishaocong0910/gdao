...
func MockBaseDao[T orm.Entity](r *require.Assertions, logicalDelCfg *LogicalDelCfg) (*baseDao[T], sqlmock.Sqlmock) {
	...
	r.NoError(err)
	builder := BaseDaoBuilder[T]()
	if logicalDelCfg != nil {
		builder.LogicalDelCfg(logicalDelCfg)
	}
	dao := builder.Build()
	orm.Config(orm.Cfg{DefaultDB: db, Logger: Logger{}, SqlLogLevel: orm.LogLevel_.INFO})
	return dao, mock
}
